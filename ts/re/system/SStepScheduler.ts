...
import { LTOStep, RunInfo } from "../objects/LScheduler";
...
export class SStepScheduler {
    ...
    public constructor(scheduler: SScheduler) {
        ...
        this._stepPhase = SStepPhase.SequenceStarting;
        this._phases = [
    ...
    public startRun(): void {
        ...
        REGame.scheduler._currentPhaseIndex = -1;
        this._stepPhase = SStepPhase.SequenceStarting;
    }
    ...
    private currentRun(): RunInfo {
        return REGame.scheduler.currentRun();
    }

    private currentStep(): LTOStep {
        const run = this.currentRun();
        return run.steps[REGame.scheduler._currentStep];
    }
    ...
    private processCore2(): void {
        switch (this._stepPhase) {
            case SStepPhase.SequenceStarting:
    ...
    private process_PhaseStarting(): void {
        REGame.scheduler._currentStep = -1;
        REGame.scheduler._currentPhaseIndex++;
        if (REGame.scheduler._currentPhaseIndex >= this._phases.length) {
            // 全 Phase 実行完了
            this._stepPhase = SStepPhase.Closed;
        }
        else {
    ...
    private process_StepStarting(): void {
        ...
        assert(REGame.scheduler._currentStep < run.steps.length);   // この時点では必ず実行するべき step があるはず
    ...
    private process_MainProcess(): void {
        ...
        if (step.isValid()) {
            ...
            phase.onProcess(this._scheduler, unit);
        }
    ...
    private process_StepClosing(): void {
        ...
        if (step.isIterationClosed()) {
            const run = this.currentRun();
            if (REGame.scheduler._currentStep + 1 < run.steps.length) {
                // まだ実行するべき Step がある
                this._stepPhase = SStepPhase.StepStarting;
            }
            else {

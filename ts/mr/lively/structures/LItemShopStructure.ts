...
export class LItemShopStructure extends LStructure {
    ...
    public getLossItems(): LEntity[] {
        ...
        const room = MRLively.mapView.currentMap.room(this._roomId);
        for (const id of this._sellngItems) {
            const item = MRLively.world.entity(id);
            if (item.floorId.equals(MRLively.mapView.currentMap.floorId()) && room.contains(item.mx, item.my)) {
                const block = MRLively.mapView.currentMap.block(item.mx, item.my);

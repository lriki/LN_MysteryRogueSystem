...
test("system.Inventory.Sort", () => {
    ...
    const grass1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_スピードドラッグ_A").id, [], "grass1"));
    const grass2 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_パワードラッグ_A").id, [], "grass2"));
    const grass3 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_グロースドラッグ_A").id, [], "grass3"));
    const weapon1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_ゴブリンのこん棒_A").id, [], "weapon1"));
    const weapon2 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_ゴールドソード_A").id, [], "weapon2"));
    const weapon3 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_セイクリッドセイバー_A").id, [], "weapon3"));
    const arrow1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_アイアンアロー_A").id, [], "arrow1").withStackCount(1));
    const arrow2 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_アイアンアロー_A").id, [], "arrow2").withStackCount(2));

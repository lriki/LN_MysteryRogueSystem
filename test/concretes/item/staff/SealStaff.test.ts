...
test("concretes.item.staff.SealStaff.basic", () => {
    TestEnv.newGame();
    const stateId = MRData.getState("kState_System_Seal").id;
    ...
    // アイテム 入手
    const item1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_シールの杖_A").id, [], "item1"));

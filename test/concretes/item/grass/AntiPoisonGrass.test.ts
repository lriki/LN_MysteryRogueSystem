...
test("concretes.item.grass.AntiPoisonGrass.player", () => {
    ...
    const item1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_アンチポイズン_A").id, [], "item1"));
    const item2 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_アンチポイズン_A").id, [], "item2"));

...
test("concretes.item.scroll.PlatingScroll", () => {
    ...
    // item1
    const item1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_プレートスクロール_A").id, [], "item1"));
    const item2 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_プレートスクロール_A").id, [], "item1"));
    const item3 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_プレートスクロール_A").id, [], "item1"));
    const item4 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_プレートスクロール_A").id, [], "item1"));

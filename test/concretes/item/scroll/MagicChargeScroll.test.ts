...
test("concretes.item.scroll.MagicChargeScroll", () => {
    ...
    // item
    const item1 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_マジックスクロール_A").id, [], "item1"));
    const item2 = SEntityFactory.newEntity(DEntityCreateInfo.makeSingle(MRData.getEntity("kEntity_リープの杖_A").id, [], "item1"));

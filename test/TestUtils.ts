...
export class TestUtils {
    ...
    public static testCommonFood(item: LEntity) {
        ...
        // 食べた時に FP を回復する効果がある？
        expect(!!reaction.emittors().find(e => !!e.effectSet.targetEffect(0).parameterQualifyings.find(x => x._parameterId == MRBasics.params.fp)));
    }
    ...
    public static testCommonGrassBegin(actor: LEntity, item: LEntity) {
        ...
        // 食べた時に FP を回復する効果がある？
        expect(!!reaction.emittors().find(e => !!e.effectSet.targetEffect(0).parameterQualifyings.find(x => x._parameterId == MRBasics.params.fp && x.formula == "500")));
